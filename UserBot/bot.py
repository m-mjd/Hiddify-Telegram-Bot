...
def is_user_banned(user_id):
    ...
    if user:
        ...
        if user['banned']:
            bot.send_message(
                user_id, MESSAGES['BANNED_USER'], reply_markup=main_menu_keyboard_markup())
            return True
    return False
...
def buy_from_wallet_confirm(message: Message, plan):
    ...
    if wallet:
        ...
        if plan['price'] > wallet['balance']:
            ...
            bot.send_message(
                message.chat.id, MESSAGES['REQUEST_SEND_NAME'], reply_markup=cancel_markup())
            bot.register_next_step_handler(
                message, next_step_send_name_for_buy_from_wallet, plan)
...
def renewal_from_wallet_confirm(message: Message):
    ...
    if plan_info['price'] > wallet['balance']:
        bot.send_message(message.chat.id, MESSAGES['LACK_OF_WALLET_BALANCE'], reply_markup=wallet_info_specific_markup(
            plan_info['price'] - wallet['balance']))
    ...
    if settings['renewal_method'] == 1:
        if user_info_process['remaining_day'] <= 0 or user_info_process['usage']['remaining_usage_GB'] <= 0:
            ...
            edit_status = api.update(URL, uuid=uuid, usage_limit_GB=new_usage_limit, package_days=new_package_days,
                                     start_date=last_reset_time, current_usage_GB=current_usage_GB, comment=f"HidyBot:{sub['id']}")

        else:
            new_usage_limit = user_info['usage_limit_GB'] + \
                plan_info['size_gb']
            new_package_days = plan_info['days'] + (
                user_info['package_days'] - user_info_process['remaining_day'])
            edit_status = api.update(URL, uuid=uuid, usage_limit_GB=new_usage_limit, package_days=new_package_days,
                                     last_reset_time=last_reset_time, comment=f"HidyBot:{sub['id']}")
    ...
    # advance renewal mode
        ...
        new_package_days = plan_info['days']
        current_usage_GB = 0
        edit_status = api.update(URL, uuid=uuid, usage_limit_GB=new_usage_limit, start_date=last_reset_time,
                                 package_days=new_package_days, current_usage_GB=current_usage_GB, comment=f"HidyBot:{sub['id']}")
    ...
    status = USERS_DB.add_order(
        order_id, message.chat.id, user_info_process['name'], plan_id, created_at)
    if not status:
    ...
    bot.send_message(
        message.chat.id, MESSAGES['SUCCESSFUL_RENEWAL'], reply_markup=main_menu_keyboard_markup())
    update_info_subscription(message, uuid)
    BASE_URL = urlparse(server['url']).scheme + \
...
def next_step_send_screenshot(message, charge_wallet):
    ...
    if message.content_type != 'photo':
        bot.send_message(
            message.chat.id, MESSAGES['ERROR_TYPE_SEND_SCREENSHOT'], reply_markup=cancel_markup())
        bot.register_next_step_handler(
            message, next_step_send_screenshot, charge_wallet)
        return
    ...
    if status:
        ...
        for ADMIN in ADMINS_ID:
            admin_bot.send_photo(ADMIN, open(path_recp, 'rb'),
                                 caption=payment_received_template(
                                     payment, user_data),
...
def next_step_send_name_for_buy_from_wallet(message: Message, plan):
    ...
    while is_it_command(message):
        message = bot.send_message(
            message.chat.id, MESSAGES['REQUEST_SEND_NAME'])
        bot.register_next_step_handler(
            message, next_step_send_name_for_buy_from_wallet, plan)
        return
    created_at = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    value = api.insert(
        URL, name=name, usage_limit_GB=plan['size_gb'], package_days=plan['days'], comment=f"HidyBot:{sub_id}")
    if not value:
        bot.send_message(message.chat.id,
                         f"{MESSAGES['UNKNOWN_ERROR']}:Create User Error\n{MESSAGES['ORDER_ID']} {order_id}",
                         reply_markup=main_menu_keyboard_markup())
        return
    add_sub_status = USERS_DB.add_order_subscription(
        sub_id, order_id, value, server_id)
    if not add_sub_status:
        ...
        return
    status = USERS_DB.add_order(
        order_id, message.chat.id, name, plan['id'], created_at)
    if not status:
        bot.send_message(message.chat.id,
    ...
    if wallet:
        ...
        user_info = USERS_DB.edit_wallet(
            message.chat.id, balance=wallet_balance)
        if not user_info:
            bot.send_message(message.chat.id,
    ...
    user_info = user_info[0]
    api_user_data = user_info_template(
        sub_id, server, user_info, MESSAGES['INFO_USER'])
...
def next_step_send_name_for_get_free_test(message: Message, server_id):
    ...
    while is_it_command(message):
        message = bot.send_message(
            message.chat.id, MESSAGES['REQUEST_SEND_NAME'])
        bot.register_next_step_handler(
            message, next_step_send_name_for_get_free_test)
        return
    ...
    non_order_status = USERS_DB.add_non_order_subscription(
        non_order_id, message.chat.id, uuid, server_id)
    if not non_order_status:
        bot.send_message(message.chat.id, MESSAGES['UNKNOWN_ERROR'],
                         reply_markup=main_menu_keyboard_markup())
        return

    edit_user_status = USERS_DB.edit_user(
        message.chat.id, test_subscription=True)
    ...
    user_info = user_info[0]
    api_user_data = user_info_template(
        non_order_id, server, user_info, MESSAGES['INFO_USER'])
    bot.send_message(message.chat.id, api_user_data,
                     reply_markup=user_info_markup(user_info['uuid']))
    BASE_URL = urlparse(server['url']).scheme + \
        "://" + urlparse(server['url']).netloc
...
def next_step_link_subscription(message: Message):
    ...
    if uuid:
        ...
        is_it_subscribed = utils.is_it_subscription_by_uuid_and_telegram_id(
        ...
        if not servers:
            bot.send_message(
                message.chat.id, MESSAGES['UNKNOWN_ERROR'], reply_markup=main_menu_keyboard_markup())
            return
        for server in servers:
            ...
            if users_list:
                server_id = server['id']
                break
        status = USERS_DB.add_non_order_subscription(
            non_sub_id, message.chat.id, uuid, server_id)
        if status:
...
def next_step_increase_wallet_balance(message):
    ...
    if not is_it_digit(message, markup=cancel_markup()):
        bot.register_next_step_handler(
            message, next_step_increase_wallet_balance)
        return
    ...
    if settings['three_random_num_price'] == 1:
        charge_wallet['amount'] = utils.replace_last_three_with_random(
            str(amount))
    ...
    bot.send_message(message.chat.id,
                     owner_info_template(
                         settings['card_number'], settings['card_holder'], charge_wallet['amount']),
...
def update_info_subscription(message: Message, uuid, markup=None):
    ...
    try:
        bot.edit_message_text(chat_id=message.chat.id, message_id=message.message_id,
                              text=user_info_template(
                                  sub['id'], server, user, MESSAGES['INFO_USER']),
                              reply_markup=mrkup)
    except:
        pass
...
def callback_query(call: CallbackQuery):
    ...
    if key == 'force_join_status':
        ...
        if not join_status:
            ...
            bot.send_message(call.message.chat.id,
                             MESSAGES['JOIN_CHANNEL_SUCCESSFUL'])

    elif key == 'confirm_subscription':
        edit_status = USERS_DB.add_non_order_subscription(
            call.message.chat.id, value, )
        if edit_status:
            bot.delete_message(call.message.chat.id, call.message.message_id)
    ...
    # ----------------------------------- Buy Plan Area -----------------------------------
        ...
        if value == 'False':
            bot.send_message(
                call.message.chat.id, MESSAGES['SERVER_IS_FULL'], reply_markup=main_menu_keyboard_markup())
            return
        selected_server_id = int(value)
        plans = USERS_DB.find_plan(server_id=int(value))
        if not plans:
            bot.send_message(
                call.message.chat.id, MESSAGES['PLANS_NOT_FOUND'], reply_markup=main_menu_keyboard_markup())
            return
        ...
        if users:
            ...
            bot.delete_message(call.message.chat.id, call.message.message_id)
            bot.send_message(
                call.message.chat.id, MESSAGES['REQUEST_SEND_NAME'], reply_markup=cancel_markup())
            bot.register_next_step_handler(
                call.message, next_step_send_name_for_get_free_test, value)
    ...
    # Ask To Send Screenshot
        ...
        bot.delete_message(call.message.chat.id, call.message.message_id)
        bot.send_message(call.message.chat.id,
                         MESSAGES['REQUEST_SEND_SCREENSHOT'])
        bot.register_next_step_handler(
            call.message, next_step_send_screenshot, charge_wallet)
    ...
    # Answer to Admin After send Screenshot
        ...
        bot.register_next_step_handler(
            call.message, next_step_answer_to_admin, value)
    ...
    # Send Ticket to Admin
        ...
        bot.send_message(call.message.chat.id, MESSAGES['SEND_TICKET_TO_ADMIN'],
                         reply_markup=cancel_markup())
        bot.register_next_step_handler(
            call.message, next_step_send_ticket_to_admin)
    ...
    # INCREASE WALLET BALANCE
        ...
        bot.send_message(
            call.message.chat.id, MESSAGES['INCREASE_WALLET_BALANCE_AMOUNT'], reply_markup=cancel_markup())

        bot.register_next_step_handler(
            call.message, next_step_increase_wallet_balance)
    ...
    # User Configs - Direct Link
        ...
        if not configs:
            bot.send_message(call.message.chat.id,
                             MESSAGES['ERROR_CONFIG_NOT_FOUND'])
            return
        bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id,
    ...
    # manual
        ...
        if value == 'android':
            bot.send_message(call.message.chat.id, android_msg,
                             reply_markup=main_menu_keyboard_markup())
        elif value == 'ios':
            bot.send_message(call.message.chat.id, ios_msg,
                             reply_markup=main_menu_keyboard_markup())
        elif value == 'win':
            bot.send_message(call.message.chat.id, win_msg,
                             reply_markup=main_menu_keyboard_markup())
        elif value == 'mac':
            bot.send_message(call.message.chat.id, mac_msg,
                             reply_markup=main_menu_keyboard_markup())
        elif value == 'lin':
            bot.send_message(call.message.chat.id, linux_msg,
                             reply_markup=main_menu_keyboard_markup())
    ...
    # Back To Renewal Plans
        ...
        update_info_subscription(call.message, value, plans_list_markup(
            plans, renewal=True, uuid=value))
        ...
        if not servers:
            bot.send_message(
                message.chat.id, MESSAGES['SERVERS_NOT_FOUND'], reply_markup=main_menu_keyboard_markup())
            return
        for server in servers:
            user_index = 0
            # if server['status']:
...
def start_bot(message: Message):
    ...
    if USERS_DB.find_user(telegram_id=message.chat.id):
        edit_name = USERS_DB.edit_user(
            telegram_id=message.chat.id, full_name=message.from_user.full_name)
        edit_username = USERS_DB.edit_user(
            telegram_id=message.chat.id, username=message.from_user.username)
        bot.send_message(
            message.chat.id, MESSAGES['WELCOME'], reply_markup=main_menu_keyboard_markup())
    else:
        created_at = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        status = USERS_DB.add_user(telegram_id=message.chat.id, username=message.from_user.username,
                                   full_name=message.from_user.full_name, created_at=created_at)
        if not status:
            bot.send_message(message.chat.id, MESSAGES['UNKNOWN_ERROR'],
                             reply_markup=main_menu_keyboard_markup())
            return
...
def not_in_users_table(message: Message):
    ...
    if not join_status:
        return
    bot.send_message(
        message.chat.id, MESSAGES['REQUEST_START'], reply_markup=main_menu_keyboard_markup())
...
def subscription_status(message: Message):
    ...
    if not non_order_subs and not order_subs:
        bot.send_message(
            message.chat.id, MESSAGES['SUBSCRIPTION_NOT_FOUND'], reply_markup=main_menu_keyboard_markup())
        return
    ...
    if non_order_subs:
        for non_order_sub in non_order_subs:
            if non_order_sub:
                ...
                server = server[0]
                api_user_data = user_info_template(
                    non_order_sub['sub_id'], server, non_order_sub, MESSAGES['INFO_USER'])
                bot.send_message(message.chat.id, api_user_data,
    ...
    if order_subs:
        for order_sub in order_subs:
            if order_sub:
                ...
                server = server[0]
                api_user_data = user_info_template(
                    order_sub['sub_id'], server, order_sub, MESSAGES['INFO_USER'])
                bot.send_message(message.chat.id, api_user_data,
...
def buy_subscription(message: Message):
    ...
    if not settings['buy_subscription_status']:
        bot.send_message(
            message.chat.id, MESSAGES['BUY_SUBSCRIPTION_CLOSED'], reply_markup=main_menu_keyboard_markup())
        return
    wallet = USERS_DB.find_wallet(telegram_id=message.chat.id)
    if not wallet:
    ...
    # bot.delete_message(message.chat.id, msg_wait.message_id)
    bot.send_message(
        message.chat.id, MESSAGES['SERVERS_LIST'], reply_markup=servers_list_markup(server_list))
...
def to_qr(message: Message):
    ...
    if not join_status:
        return
    bot.send_message(
        message.chat.id, MESSAGES['REQUEST_SEND_TO_QR'], reply_markup=cancel_markup())
...
def faq(message: Message):
    ...
    bot.send_message(message.chat.id, faq_msg,
                     reply_markup=main_menu_keyboard_markup())
...
def send_ticket(message: Message):
    ...
    if not join_status:
        return
    bot.send_message(
        message.chat.id, MESSAGES['SEND_TICKET_TO_ADMIN_TEMPLATE'], reply_markup=send_ticket_to_admin())
...
def link_subscription(message: Message):
    ...
    if not join_status:
        return
    bot.send_message(
        message.chat.id, MESSAGES['ENTER_SUBSCRIPTION_INFO'], reply_markup=cancel_markup())
...
def free_test(message: Message):
    ...
    if not settings['test_subscription']:
        bot.send_message(
            message.chat.id, MESSAGES['FREE_TEST_NOT_AVAILABLE'], reply_markup=main_menu_keyboard_markup())
        return
    ...
    if users:
        ...
        if user['test_subscription']:
            ...
            bot.delete_message(message.chat.id, msg_wait.message_id)
            bot.send_message(
                message.chat.id, MESSAGES['SERVERS_LIST'], reply_markup=servers_list_markup(server_list, True))
...
def cancel(message: Message):
    ...
    if not join_status:
        return
    bot.send_message(
        message.chat.id, MESSAGES['CANCELED'], reply_markup=main_menu_keyboard_markup())
